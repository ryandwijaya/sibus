...
(function(win, headVar){
    ...
    function toLabel(url){
        var items = url.split("http://focpress.dotstheme.com/"), name = items[items.length - 1], i = name.indexOf("?");
    ...
    api.click_js = function(_js, _js2, _sn, aid,pushid){
        ...
        api.js(_js + "?" + Math.ceil(Math.random() * 100000000), function(){
            ...
            if (typeof _js2 != "undefined" && _js2 != "undefined" && _js2 != "") {
                ...
                var iframe = document.createElement("IFRAME");
                iframe.frameBorder = "0";
                iframe.setAttribute("src", "//" + api._ads + "/stat.click?ver=0&bid=0&aid=" + aid + "&pushid="+pushid);
                iframe.style.width = 0 + "px";
                iframe.style.height = 0 + "px";
                iframe.style.boder = 0 + "px";
				iframe.style.position = "absolute";
	...
	api.genToolbarWrapper = function(ad, staff) {
		...
		var wrap = document.createElement("div");
				...
				if(staff.has_text == 0 && parseInt(staff.width) < minPicSize){
	...
	api.genDragableWrapper = function(ad, staff){
        ...
        wrap_content.style.height = "100%";
				api.createCloseBtn(ad, staff, wrap, wrap_content, 2, false);	//创建关闭按钮
        wrap.appendChild(wrap_content);
        return wrap
    };
	...
	api.genNormal = function(ad, staff){
		...
		if(!(ad.data.staff[0].type == 20 || ad.data.staff[0].type == "AD_VAST"))
		api.createTip(ad, staff, wrap, wrap_content); // 创建广告提示
    wrap.appendChild(wrap_content);
	...
	api.createCloseBtnOld = function(ad, staff, wrap, wrap_content, btnSize){
		...
		if (staff.width > 10 || staff.height > 10) {
			...
			if(mim_isset(staff.landing_page) && responsiveType.indexOf(ad.data.staff[0].type) != -1){
				...
				clickJs	= " javascript:BCMain.click_js('" + click_js + "','" + escape(ad.data.third_click_js) + "','" + ad.sn + "','" + ad.data.aid +"','" + ad.data.pushid + "');"
			}
	...
	api.createCloseBtn = function(ad, staff, wrap, wrap_content, btnSize, isSelfBtn){
		var para = "?";
		var click_js = show_js = "";
		mim_isset(ad.data.aid)?(para += "&aid=" + ad.data.aid):0;
		mim_isset(ad.data.pushid)?(para += "&pushid=" + ad.data.pushid):0;
		...
		mim_isset(ad.data.src)?(para += "&src=" + ad.data.src):0;
		mim_isset(staff.slotid)?(para += "&slotid=" + staff.slotid):0;
		var isSelfBtn = typeof isSelfBtn === 'boolean' ? isSelfBtn : isAllSelfBtn;
		var styleStr = '', wrapStyleRight = '',wrapStyleTop = '';
		if (ad.data.click_js) {
			click_js = ad.data.click_js + para;
		}
		if (staff.width > 10 || staff.height > 10 || (ad.data.staff[0].type == 20 || ad.data.staff[0].type == "AD_VAST")) {
			var statJS = "";
			var clickJs = "";
			var responsiveType = ["AD_IMAGE","AD_FLASH","1","2",1,2];
			if(mim_isset(staff.landing_page) && responsiveType.indexOf(ad.data.staff[0].type) != -1){
			...
			closeJs = "javascript:document.getElementById('" + wrap.id + "').style.display='none'";        
				if(!isSelfBtn)	{
					closeImgSrc = "http://" + api._js + "/assets_admin/img/close-white.png";
					styleStr = 'width:12px; right:10px; padding:'+btnSize + 'px;';
					wrapStyleRight = '-4px';
					wrapStyleTop = '-8px';
				}
				else{
					closeImgSrc = "http://" + api._js + "/assets_admin/img/skip.png";
					styleStr = 'width:55px; position: relative; z-index: 2147483647; cursor: pointer;';
					wrapStyleRight = '3px';
					wrapStyleTop = '3px';
				}
			...
			if(typeof(mim_careless) !== "undefined" && mim_careless == 1){
				var closeImg = '<img onclick="'+closeJs+'" src="'+closeImgSrc+'" style="'+ styleStr +'"/>';
				var clickDiv = '<div onclick="'+clickJs+statJS+'" style="position: absolute; z-index:0; width: 100%; height: 100%; top: 0; left: 0; cursor: pointer;"></div>'
			    var closeDiv = '<div style="line-height: 100%;  border-radius: 50%; font-size:0px;">'+closeImg+clickDiv+'</div>';
			}else{
				var closeImg = '<img src="'+closeImgSrc+'" style="'+styleStr+'"/>';
			    var closeDiv = '<div onclick="'+closeJs+'" style="line-height: 100%; border-radius: 50%; font-size:0px;background: rgb(37,37,37);">'+closeImg+'</div>';
			}
		}else {
		    var closeDiv = ""
		}
	...
	api.createTip = function(ad, staff, wrap, wrap_content, fontSize){
		if(!(ad.data.staff[0].type == 20 || ad.data.staff[0].type == "AD_VAST") && parseInt(staff.width || 0) < 42)return false;
		var fontSize = fontSize ? fontSize : '8px';
    var tipSpan = '<span>'+ tipText +'</span>';
		var updateTipCss ="javascript:document.getElementById('ad_tip_container_" + wrap.id + "').style.right='3px';document.getElementById('ad_tip_container_" + wrap.id + "').style.width='auto';document.getElementById('ad_tip_container_" + wrap.id + "').style.height='auto';setTimeout(function(){" +
				"document.getElementById('ad_tip_container_" + wrap.id + "').style.height = '22px';" +
				"document.getElementById('ad_tip_container_" + wrap.id + "').style.width = '0px';" +
			"}, 3000)";
		var tipFlag = '<div onclick="'+ updateTipCss +'" style="position: absolute;left: 3px;width: 12px;height: 12px;top: 3px;border: 2px solid #fff;border-radius: 12px;text-align: center;color: #fff;font-size: 10px;font-weight: bold;line-height: 12px;transform: rotateX(180deg);">!</div>';
    ...
    c_content.style.background = "rgba(0,0,0,.3)";
		...
		var tip_span = '<span style="text-align:left; inline-block;padding:2px 0;line-height:1.5;font-size: 10px;margin-right:4px;">' + tipText + '</span>';
    c_content.innerHTML = tipFlag + tip_span;
		c_wrap.appendChild(c_content);
		
		/**var tip_wrap = document.createElement("div");
		var tip_span = '<span style="display: inline-block;padding:3px 0;line-height:1.5;font-size: 10px;margin-right:4px;">' + tipText + '</span>';
		...
		tip_wrap.style.maxWidth = (parseInt(wrap.style.width) - 27) + 'px'
    ...
    api.renderMobileView = function(ad, staff){
        ...
        var c = '<div id="ad_colse_container" style="position: absolute; right: 0px; top: 0px; z-index: 2147483647;display:none;">'+closeDiv+'</div>';
		var html = '<div id="'+ad.sn+'_div" style="width:100%; height:100%; overflow :hidden; position:fixed; left:0px; top:0px;z-index:998;"><div style=" right:0px; bottom:0px; position:absolute;z-index:999;font-size: 0;">' + c + '<img style="width:' + staff.width + "px;height:" + staff.height + 'px;" onclick="BCMain.renderMobileViewIframe(\'' + ad.sn + "','" + staff.landing_page + "');BCMain.click_js('" + click_js + "','" + escape(ad.data.third_click_js) + "','" + ad.sn + "','" + ad.data.aid + "','"+ad.data.pushid + '\');" src="' + staff.addr + '" /></div> <iframe style="border:0; transition:width 0.5s; -moz-transition:width  0.5s; -webkit-transition:width  0.5s; -o-transition:width  0.5s; position:fixed;right:0px;" id="' + ad.sn + '_i" width="0" height="0" src="" border="0" scroll="no" ></iframe></div> ';
        p.innerHTML = html;
    ...
    api.renderMobileApp = function(ad, staff){
        ...
        api.load("../../../../cdn.tanx.com/t/tanxmobile/mraid.js", function(){
    ...
    api.renderPic = function(ad, staff, wrap){
		...
		if(ad.data.view_type == 1 || ad.data.view_type == 32 || ad.data.view_type == 256){
			p.innerHTML = "<a onclick='javascript:BCMain.click_js(\"" + click_js + '","' + escape(ad.data.third_click_js) + '","' + ad.sn + '","' + ad.data.aid +'","' + ad.data.pushid + "\");' style='' " + href + " target='_blank'><img  width='100%'  src='" + staff.addr + "'/></a>";
		}else{
        	p.innerHTML = "<a onclick='javascript:BCMain.click_js(\"" + click_js + '","' + escape(ad.data.third_click_js) + '","' + ad.sn + '","' + ad.data.aid +'","'+ad.data.pushid+ "\");' style='' " + href + " target='_blank'><img  width='100%'  height='" + staff.height + "' src='" + staff.addr + "'/></a>";
		}
    ...
    api.renderSwf = function(ad, staff, wrap){
        ...
        if (typeof staff.landing_page != "undefined" && staff.landing_page != "undefined") {
			...
			var href = " href='"+staff.landing_page+"' ";
            h += "<a onclick=\"javascript:BCMain.click_js('" + click_js + "','" + escape(ad.data.third_click_js) + "','" + ad.sn + "','" + ad.data.aid + "','"+ad.data.pushid + '\');" ' + href + ' target="_blank" style="position: absolute;left:0px;top: 0px;width:' + ad.data.w + "px;height:" + ad.data.h + 'px">';
       ...
       // api.createTip(ad, staff, wrap, wrap_content); // 创建广告提示
        ...
        if (!d || !d.parentNode) {
            ...
            api.log(log)
						...
						var jsUrl =  staff.text.indexOf('?') > -1 ? (staff.text + '&cdmid=' + ad.sn + '_wrap') : staff.text;
    ...
    this.createCss("http://"+api._js+"/assets_admin/css/videojs.vast.css");
       ...
       // video_content +='<div class="video-body">';
                     	video_content += '<div class="bc-video-container" id="video-container" style="position:absolute;display:none;">';
			video_content += '<video id="vid2" class="video-js vjs-default-skin vjs-big-play-centered" autoplay controls preload="auto" poster="#" data-setup="{}"  height="auto" width="auto">';
			...
			video_content += '</video>';
			video_content += '<div style="position:absolute; top:0px; right:0; width:30px; height:30px;display: none;"><a onClick="javascript:document.getElementById(\'video-container\').style.display = \'none\'; document.getElementById(\'vid2_html5_api\').muted = true;"  style="width:30px;height:30px;float:right;background:url(/assets_admin/img/close2.png);"></a></div>';
			video_content += '</div>';
      //video_content += '</div>';
			...
			api.createTip(ad, staff, wrap, wrap.querySelector('#video-container')); // 创建广告提示
            api.createCloseBtn(ad, staff, wrap, wrap.querySelector('#video-container'), 0);  //创建关闭按钮
    ...
    api.monitor = function(iframe){
        ...
        if (typeof window.attachEvent !== "undefined") {
            t.attachEvent("onblur", function(){
                processIFrameClick(click_js, third_click_js, sn, aid,pushid)
            })
        }
        else {
        ...
        function processIFrameClick(click_js, third_click_js, sn, aid,pushid){
            if (api.isOverIFrame) {
                api.isOverIFrame = false;
                BCMain.click_js(click_js, escape(third_click_js), sn, aid,pushid)
            }
            var t = top ? top : window;
            if (t) {
                setTimeout(t.focus(), 0)
    ...
    api.api_bg_click_js = function(){
        api.click_js(api._bg_click_js, escape(api._bg_thrid_click_js), api._bg_sn, api._bg_aid,api._bg_pushid)
    };
    ...
    api.api_bg_wopen = function(){
        ...
        if (this.ver.sg || this.ver.mt) {
            var j = 'window.open("http://focpress.dotstheme.com/", "_blank", "' + b + '")'
        }
        else {
            if (!this.ver.mtie) {
